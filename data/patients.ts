...
const ParseEntries = ():Patient[] => {
    let parsedEntries: Patient[] = [];
    patients.map(pat => {
        pat.entries.map(entry => {
            if (entry.type in EntryType){
                return {...entry, type: entry.type as EntryType};
            }
            throw new Error('Bad entry type');
        });
        parsedEntries = parsedEntries.concat(pat as Patient);
    });
    return parsedEntries;
};

const patientEntries = ParseEntries();
